...
def is_threat(aircraft_class):
    ...
    military_keywords = [
        ...
        "WZ10",
        "WZ7",
        "WZ9",  # Chinese attack helicopters
        ...
        "An124",
        "An22",
        "An225",  # Antonov military transports
...
def draw_detection(img, x1, y1, x2, y2, aircraft_type, confidence, is_threat_detected):
    ...
    cv2.rectangle(
        img, (x1, y1 - text_height - baseline - 10), (x1 + text_width, y1), color, -1
    )
...
def draw_status_banner(img, threat_detected, num_detections):
    ...
    cv2.putText(
        img, status_text, (20, 50), cv2.FONT_HERSHEY_SIMPLEX, 1.2, TEXT_COLOR, 3
    )
