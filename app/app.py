...
possible_paths = [
    ...
    PROJECT_ROOT
    / "results/model_1_output/augmented_on_fly/weights/best.pt",  # YOLOv8n augmented (fallback)
    PROJECT_ROOT
    / "results/model_2_output/yolov8s_baseline2/weights/best.pt",  # YOLOv8s baseline
    PROJECT_ROOT
    / "results/model_1_output/baseline_no_aug2/weights/best.pt",  # YOLOv8n baseline
    PROJECT_ROOT
...
def process_frame(frame):
    ...
    results = model.predict(
        img_bgr, conf=CONFIDENCE_THRESHOLD, iou=IOU_THRESHOLD, verbose=False
    )
    ...
    if result.boxes is not None and len(result.boxes) > 0:
        for box in result.boxes:
            ...
            annotated_img = draw_detection(
                annotated_img, x1, y1, x2, y2, aircraft_type, confidence, threat
            )
...
with gr.Blocks(title="Aircraft Threat Detection") as app:
    ...
    with gr.Row():
        ...
        with gr.Column(scale=2):
            ...
            video_display = gr.Image(
                label="Live Detection Feed", type="numpy", height=480, show_label=True
            )
            ...
            with gr.Row():
                ...
                stop_btn = gr.Button(
                    "⏹️ Stop", elem_classes="blue-button", size="lg", scale=1
                )
            ...
            image_input = gr.Image(
                label="Upload Aircraft Image", type="numpy", height=300
            )
            detect_btn = gr.Button(
                "🔍 Analyze Image", elem_classes="blue-button", size="lg"
            )
            ...
            process_video_btn = gr.Button(
                "🎥 Process Video", elem_classes="blue-button", size="lg"
            )
            ...
            video_output = gr.Video(
                label="Processed Video with Detections", width=640, height=480
            )
    ...
    def start_stream():
        ...
        if not video_capture.isOpened():
            return (
                "❌ Could not open webcam. Please check your camera permissions.",
                None,
            )
        ...
        return (
            "✅ Video streaming started! Point your webcam at an aircraft.",
            first_frame,
        )
    ...
    def stop_stream():
        ...
        return "⏹️ Video detection stopped.", cv2.cvtColor(
            placeholder, cv2.COLOR_BGR2RGB
        )
    ...
    # Connect components
    start_btn.click(fn=start_stream, outputs=[status, video_display]).then(
        fn=stream_video, outputs=video_display
    )

    stop_btn.click(fn=stop_stream, outputs=[status, video_display])

    detect_btn.click(fn=process_image, inputs=image_input, outputs=video_display)

    process_video_btn.click(
        fn=process_video, inputs=video_input, outputs=[video_status, video_output]
    )
